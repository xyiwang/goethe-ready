...
export function ProgressPage({ messages, locale, setLocale, userId = null, days, onBack }) {
  ...
  const cellClass = (cell) => {
    ...
    if (!inMonth) {
      ...
      if (s === 'full') base += ' bg-[#6C5CE7] text-white'
      else if (s === 'partial') base += ' bg-[#E8E3FF] text-[#6C5CE7]'
      else base += ' bg-slate-300 text-slate-600'
    }

    if (isToday) {
      base += ' ring-2 ring-[#6C5CE7] ring-offset-2 ring-offset-white'
    }
    ...
    <div className="min-h-dvh bg-[#FAFAFA] px-6 py-10 pb-16 text-slate-900">
      <div className="mx-auto w-full max-w-md">
        ...
        <section className="mb-10 rounded-2xl border border-slate-200 bg-slate-50/90 px-5 py-6">
          ...
          <p className="mt-2 text-center text-5xl font-bold tabular-nums tracking-tight text-[#6C5CE7] sm:text-6xl">
            {daysUntilExam}
          </p>
          ...
          <div className="mt-8">
            <div className="mb-2 flex items-end justify-between text-sm">
              ...
              <span className="tabular-nums font-semibold text-[#6C5CE7]">{progressPct}%</span>
            </div>
            <div className="h-2.5 overflow-hidden rounded-full bg-slate-200">
              <div
                className="h-full rounded-full bg-[#6C5CE7] transition-[width] duration-500"
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-3 text-[11px] text-slate-500 sm:text-xs">
              <span className="inline-flex items-center gap-1.5">
                <span className="h-2.5 w-2.5 rounded bg-[#6C5CE7]" />
                {p.legendFull}
              </span>
              <span className="inline-flex items-center gap-1.5">
                <span className="h-2.5 w-2.5 rounded bg-[#E8E3FF]" />
                {p.legendPartial}
              </span>
              <span className="inline-flex items-center gap-1.5">
        ...
        <section className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div className="rounded-xl border border-[#FDCB6E] bg-[#FFF8E6] px-4 py-4 text-center">
            <p className="text-2xl" aria-hidden>
              🔥
            </p>
            <p className="mt-1 text-2xl font-bold tabular-nums text-[#6C5CE7]">{streak}</p>
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-4 text-center">
            ...
            <p className="mt-1 text-2xl font-bold tabular-nums text-[#6C5CE7]">{cumulativeDoneDays}</p>
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-4 text-center">
            ...
            <p className="mt-1 text-2xl font-bold tabular-nums text-[#6C5CE7]">{cumulativeWords}</p>
